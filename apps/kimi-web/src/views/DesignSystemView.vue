          ...
          <section id="rules">
            ...
            <table class="dt">
              ...
              <tbody>
                ...
                <tr><td class="tk">no-glassmorphism</td><td><code>backdrop-filter: blur</code> (one shared parametric glass system in <code>src/style.css</code> — three named tiers <code>.lg-glass</code> (8px dark / 20px light), <code>.lg-frost</code> (34px), <code>.lg-band</code> (14px), plus the always-on <code>.lg-scrim</code> overlay layer; the streaming surface stays glass-free; the three tiers are gated behind the Liquid glass toggle)</td><td><span class="pill amber">3 tiers + scrim</span></td></tr>
            ...
            <div class="callout good"><span class="ico">✓</span><div>
              <code>backdrop-filter: blur</code> is governed by <b>one shared parametric glass system</b> in <code>src/style.css</code> implementing the "D1 heavy refraction" recipe: translucent tint over a saturated blurred backdrop (<code>--lg-sat: 190%</code>, <code>--lg-bright: 1.04</code>), a specular corner bloom, an asymmetric edge rim lit from the top-left (<code>--lg-rim-*</code>, top 70% white / left 25% white / right 12% black / bottom 16% black) and a subtle chromatic double outer shadow (warm copy down-right, cool copy up-left). Tier classes only set parameters (<code>--lg-blur</code>, <code>--lg-tint-a</code>, <code>--lg-drop-shadow</code>); ONE consuming rule paints the material, and classless surfaces (the sidebar column, the chat blur bands, the right panel, the new-messages pill) consume the same variable family instead of hand-writing recipes. The three tiers are gated behind the "Liquid glass effects" settings toggle (<code>html[data-liquid-glass]</code>). <b>Tier 1</b> — <code>.lg-glass</code> (blur 8px dark / 20px light) on small floating elements (menus, dropdowns, tooltips, toasts, the TOC card). <b>Tier 2</b> — <code>.lg-frost</code> (blur 34px) on dialog/sheet panels (<code>.ui-dialog</code>, <code>.ui-sheet</code>, <code>.sheet-panel</code>, <code>.server-auth-card</code>, <code>.goal-pop</code>), the composer card, the sidebar column and the docked right panel, plus the existing <code>TopBar</code> <code>.frost</code> sticky nav bar. <b>Tier 3</b> — <code>.lg-band</code> (blur 14px) for embedded blur strips: the chat transcript's top/bottom blur bands and the dock squares (the transcript bands are pseudo-elements and consume the band variables directly). Supporting the tiers, the <b>always-on overlay scrims</b> take the shared <code>.lg-scrim</code> utility (<code>.ui-dialog__overlay</code>, <code>.ui-sheet__scrim</code>, <code>.sheet-scrim</code>, <code>.server-auth-overlay</code>, <code>.att-lightbox</code>, <code>.media-preview-overlay</code> — <code>blur(10px) saturate(140%)</code>) which defocuses the app behind floating windows and is intentionally NOT gated by the toggle. On engines where <code>backdrop-filter: url(#…)</code> parses (feature-detected via <code>@supports</code>), the <code>GlassDefs.vue</code> SVG lens (edge refraction + specular glint + subtle grain) is appended <b>only on floating overlay surfaces that also carry the <code>.lg-lens</code> marker class</b> (menus, dropdowns, tooltips, toasts, dialogs, sheets, the TOC card, the goal and dock popovers, and the code-preview frame) — the lens re-rasterizes on every frame whose backdrop changes, so a surface that scrolls with the transcript (tool-fold chips, the new-messages pill) or repaints on every keystroke (composer pills), or an always-visible large area (sidebar, right panel, blur bands, dock squares), would pay 2–3x frame time for a refraction that is barely visible there; those surfaces keep the plain blur chain plus the rim insets and dispersion pair; <code>prefers-reduced-transparency: reduce</code> collapses every tier to the solid raised surface and <code>prefers-reduced-motion: reduce</code> removes glass transitions. With the toggle off there is no glassmorphism anywhere except the always-on overlay scrims. No other component may add <code>backdrop-filter</code>; violations are flagged under <code>no-glassmorphism</code>. Glass stays banned on streaming surfaces (chat scroll, xterm).
            </div></div>
