...
describe('modelThinking', () => {
  ...
  describe('subagentEffortOptions', () => {
    it('uses the model-declared efforts when a representative model is known', async () => {
      const { subagentEffortOptions } = await import('./modelThinking');
      expect(subagentEffortOptions(effortModel)).toEqual(['low', 'high', 'max']);
    });

    it('uses the catalog fallback when no representative model is known', async () => {
      const { subagentEffortOptions } = await import('./modelThinking');
      expect(subagentEffortOptions(undefined)).toEqual(['low', 'medium', 'high', 'max']);
    });

    it('does not invent efforts for a known model without effort support', async () => {
      const { subagentEffortOptions } = await import('./modelThinking');
      expect(subagentEffortOptions(booleanModel)).toEqual([]);
    });

    it('keeps a stored effort that is no longer declared by the model', () => {
      expect(subagentEffortOptions(effortModel, 'ultra')).toEqual(['low', 'high', 'max', 'ultra']);
    });
  });

  describe('modelThinkingInfoFromConfig', () => {
    it('converts config-only model metadata from camelCase and snake_case', () => {
      expect(modelThinkingInfoFromConfig({
  ...
  describe('representativeModelForSubagent', () => {
    it('uses secondary for an unpinned profile and preserves explicit primary', () => {
      const primary = model({ capabilities: ['thinking'], supportEfforts: ['high'] });
      const secondary = model({ capabilities: ['thinking'], supportEfforts: ['low'] });
      const resolve = (alias: string) => alias === 'primary' ? primary : secondary;

      expect(representativeModelForSubagent(undefined, undefined, primary, secondary, resolve)).toBe(secondary);
      expect(representativeModelForSubagent('primary', undefined, primary, secondary, resolve)).toBe(primary);
      expect(representativeModelForSubagent('secondary', undefined, primary, secondary, resolve)).toBe(secondary);
    });

    it('preserves an explicit pinned alias over profile preference', () => {
      const pinned = model({ capabilities: ['thinking'], supportEfforts: ['max'] });
      const primary = model({ capabilities: ['thinking'], supportEfforts: ['high'] });
      expect(representativeModelForSubagent('primary', 'pinned', primary, undefined, (alias) => alias === 'pinned' ? pinned : undefined)).toBe(pinned);
    });
  });
