...
- `apps/kimi-web`: the browser web UI, a peer to the TUI. Vue 3 + Vite + vue-i18n; talks to the server over REST + WebSocket under `/api/v1`. **Fork note:** upstream deleted this directory (it now ships a prebuilt `dist-web` bundle), but this fork keeps the in-tree source and the `pnpm -C apps/kimi-web run build` + `apps/kimi-code/scripts/copy-web-assets.mjs` pipeline — do not adopt upstream's `sync:web` / `check-web-assets.mjs` flow here. It must not depend on engine packages (wire types are re-implemented locally).
