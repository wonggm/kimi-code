...
**What the pre-fix measurement showed for the three scenes.** Rebuilt the fork bundle at HEAD without this pass's two source edits (`.tmp/prefix-web`) and ran the three area scenes against it: all four of their requirement groups pass there too, so the scenes are invariants over the three areas rather than regression tests for this pass — the fork's divergence in those areas showed up only as the element and text differences the walk reports, which is what the sixteen blockers above were. The one behaviour this pass added without a scene of its own is the dock panel's `body-scrolled-up` mask; it is covered by the walk's element comparison (`DIV.body-scrolled-up.dock-work-panel.panel-goal` on the mobile goal state) and by source, not by a requirement.
