...
**Still different, deliberately or recorded:** with the tool-summary preference off upstream hides `ar-head` and the fork still shows it; and a lone `.tool-line` has no top margin in the fork. (The thinking block's placement, once recorded here as the fork keeping it as a sibling before the run, is **no longer different**: the fork now nests the thinking item inside the run's body as upstream does — see the sixth slice below.)
...
### Fifth pass, first slice: the walk was clicking through modals (2026-09-14)

The largest blocker group in the settings area (386 of 636 in the run before the fixes) is an artifact of the walk, not a fork difference. With the settings dialog open, the walk still discovered the dock pills *behind* it and clicked their coordinates. On upstream that point lands on the dialog (which swallows it, so the dialog stays open and its labels are all present); on the fork it lands on the backdrop, which closes the dialog — so the captured state was "settings dialog" on one side and "no dialog" on the other, reported as ~50 missing texts and elements per state.

Measured directly: with the dialog open, a deliberate click at the viewport corner (20, 20) closes it on **both** apps (`dialog: true, tabs: 10` → `dialog: false, tabs: 0`), so the dismissal behaviour itself matches and the earlier findings were coordinate artifacts.

The fix is in the walk: discovery now hit-tests each control's centre with `document.elementFromPoint` and skips a control that another element covers, because such a control cannot be acted on and the state it produces is not its surface. `DISCOVER_EXPR` in `webdiff/surfaces.mjs` carries the reasoning inline.

**Goal, second slice: the discovery fix's effect, and two new scenes.** Runtime 02:28–02:52, the same 16 single-app jobs: blocker **636 → 163**, warnings 15 319 → 9 627, and **zero requirement blockers** — every behavioural scene, old and new, meets its requirements on both apps. The 473 blockers the hit-test removed are the settings-walk states where the two apps were comparing "dialog open" against "backdrop clicked". What remains in the three goal areas: composer **none**, left panel 8 (4 of them `SPAN.think-title`, which is the recorded thinking-block placement divergence surfacing in sidebar-click states, plus 2 "Expand all workspaces" and 2 "Goal" in mobile states), and settings 61 — 26 of those in `mobile-dark-zh` alone, where upstream's sheet shows labels the fork's does not ("外观", "字体大小", "上下文", "思考强度", "暂不支持", the tool-summary description, a `29.3k/122k` context readout).

Two scenes were added: `behaviour-sidebar-workspace-collapse` (collapse hides every `.group-sessions`, the same control restores them — class-addressed, so it holds in both locales) and `behaviour-settings-pane` (the dialog's sections plus dismissal on an outside click). The sidebar scene passes on both apps at both stages. The settings scene currently fails on **both**, and its click executes with no gap recorded, so it is the scene's own routing rather than a fork difference: the base `settings` scene opens the same dialog through the same first route, so the next step is to compare the two scenes' acceptance mechanics (`expect` plus the runner's `baseline`) rather than to touch the fork.

**The settings scene's own bug, and its fix.** The scene failed on *both* apps with no step gap, which pointed away from the fork and at the scene. Two causes, both measured: the requirement named `.settings-dialog`, which **does not exist on either app** (upstream's class is `.settings-dialog-title` inside the tabs header), and a shortened route list did not survive a coordinate click that a tooltip swallows. The scene now uses the route list the base `settings` scene uses (`SETTINGS_ROUTES`, shared by both) and the shell both apps really share, `.ui-dialog`, for the dialog marker and the text scopes; the requirement poll went from 1.5 s to 4 s because a surface can be a lazily loaded chunk, and the capture taken two seconds after the click showed the dialog the check had just called missing. With those, the scene reports `requirements-met` on upstream and on the fork at both stages: the dialog's Appearance and Account sections, the font-size row, and dismissal on an outside click.

**Goal, third slice: the mobile settings "missing labels" were a route artifact too.** The 26 `mobile-dark-zh` settings blockers (设置 / 外观 / 字体大小 / 语言 / 权限 / 模型 / 思考强度 / 上下文 / 暂不支持 / English / 简体中文 …) came from the scene accepting the wrong surface: the base `settings` scene's `expect` was broad enough (`settings|appearance|theme|language|provider|plugin|permission|account|设置|外观|…`) that a half-open menu satisfied it, so the fork's capture was the main view while upstream's was its settings sheet. A probe confirmed the fork's mobile control is reachable and opens a sheet with 16 `srow-label` rows. Narrowing `expect` to labels only a settings surface carries (`appearance|font size|外观|字体大小`) makes the mobile-dark-zh `settings` run capture both apps' sheets (`sheet-root`, `srow-label`, 外观, 字体大小 on both) and that combination's blockers fall to **1**.

**Goal, fourth slice: the gate on the current harness** (`.tmp/wdfull22`, 03:12–03:36, 16 jobs): blocker **120** (636 → 163 → 120 across the three runs), warnings 9 443, and **zero requirement blockers**. Per area as the completion criterion counts them: composer **0**; left panel **8**; settings **18**. What those name: `SPAN.think-title` 17 times (the recorded thinking-block placement divergence — upstream nests the thinking item inside the run, the fork renders it as a sibling before the run), `expand-all-workspaces` 2, and "Goal"/"目标" 4 in states where the two apps show different surfaces. Everything else in those areas is `dom-elements-missing` with an element name, i.e. the same thinking-item pairing.

**Goal, fifth slice: the mobile work-mode chip was hidden by the fork.** Every mobile state reported upstream's "Goal" as missing (18 blockers across `mobile-dark-en`/`mobile-light-en`: `main` and each walk state). Measured both apps at a 390px viewport with the goal-mode seed: upstream's composer shows its work-mode chip (`span.wm-pill`, "Goal", 89×24 at the composer's top-left) while the fork's `.wm-pill` rendered at 0×0 — its mobile block hid `.perm-pill, .wm-pill`. Upstream hides only the permission pill there, so the fork now hides only that one and keeps the chip; measured visible again (78×21 at 29,713, text "Goal").

**Goal, sixth slice: the thinking item is nested inside the run.** The 17 `SPAN.think-title` blockers were the last large group in the left-panel and settings states, and they were the divergence recorded above. `chatTurnRendering.ts` now builds a run as a list of items (`thinking` or `tool`) and `ActivityRun.vue` renders them in order inside `.ar-body-inner` — measured class path down to the title is now `SPAN.think-title > BUTTON.think-head > DIV.think.mob > DIV.ar-body-inner > DIV.ar-body > DIV.activity-run > DIV.a-msg.turn-anchor`, byte-identical to upstream's (before: no `.activity-run` ancestor at all). A run of one item is still emitted as that item alone, so a lone thinking block stays standalone as upstream's does. Scene run `main,settings` on desktop/dark in both locales: 4 blockers naming `think-title` → **0**; with the walk enabled, 9 `think-title` findings → **0**, warnings and info identical, and the run's summary line byte-identical. Typecheck clean, 1015 tests pass (+4 for the nesting rule).

**Goal, seventh slice: the composer's scene.** `behaviour-composer-permission-menu` clicks the composer's permission pill, requires the three-row menu (`.perm-dropdown` with three `.pd-row`), then clicks the same control again and requires the menu gone — class-addressed, so it holds in both locales. Verified: 0 blockers in that scene run, `requirements-met` on upstream and on the fork at both stages. The fork's pill behaviour was checked by probe first (it opens the dropdown with three rows on a coordinate click), so the walk state upstream reached and the fork did not was a capture artifact rather than a fork gap. That gives all three areas a scene: composer, left panel, settings.

**Goal, eighth slice: the settings dialog's own rows.** Beyond the scenes, the settings pane still differed in content, and the fixes were concrete:

- **Advanced and Agent tabs** now carry upstream's rows, order, value/description layout and class names: `Version & updates` with `App version` / `Server version` / `Server address`, `Data & privacy`, `Diagnostics`, and a `Subagents` section with `Subagent model` and `Model and thinking effort that subagents use by default` / `Not set (inherit primary)`. The select primitives grew upstream's `ui-select` and `sm-picker` wrappers. Blockers on those two tabs: 30 → 8, all eight explained (a build stamp, a mock port, an inert `Auto-fold messages` toggle for a feature the fork rejected — already allowlisted — and a boolean thinking row where upstream shows a level).
- **The app version row reported the wrong thing.** It printed the web package's version (`0.1.2`) with no build time. The app whose release this row reports is the CLI, so Vite now defines that version plus a build timestamp and the row reads `0.41.0 · 2026-09-14 05:02`, upstream's format.
- **Two values can never agree between two bundles or two mock ports**, so the capture's text inventory now blanks a build stamp's date and a loopback port — the same treatment the settle signature already gives elapsed times. Without it, "App version" and "Server address" produce a fresh blocker on every build and every port.
- **The left panel's last blocker was a tooltip lifetime, not a control.** Both apps carry `Expand all workspaces` on the same `.side-section-toggle`; the fork's bubble mounts with the text 600 ms after the click while the button keeps focus, and the harness's own scroll normalisation then hides it (the fork's tooltip hides on scroll, upstream's survives). Allowlisted with that measurement.

### Sixth pass: the gate's residue in the settings pane, and the dock panel's scroll fade (2026-09-14)

Gate `.tmp/wdfull24` (05:36–05:59, sixteen single-app jobs, four-way pool, on the 05:35 bundle): blocker **16** (636 → 163 → 120 → 106 → **16**), warnings 12 467, info 12 986, suppressed 568, **zero requirement blockers**. Per area as the completion criterion counts them: composer **0**, left panel **0**, settings **12**. Every one of the sixteen names a `walk-settings-*` state (12) or falls outside the three areas (2 Chinese tooltip texts, 2 mobile dock-panel states). Each fix below was measured against the captured page source of the state that named it.

**The Lab section's own group wrapper.** In the Lab state's captured source, upstream carries ten `settings-group` wrappers against the fork's nine, and counting them per `sec-title` shows the fork's Lab section is the one section with none: the row sat directly under `sec-title`, so it lost the row separators every other section draws. `SettingsDialog.vue` now wraps it, matching the other nine sections. That was the four `walk-settings-click-06-button-lab` blockers, one per desktop combination.

**The dock panel's body fade is upstream's `body-scrolled-up`.** The two mobile blockers named `DIV.body-scrolled-up.dock-work-panel.panel-goal`. Read out of the upstream bundle, the class is **the panel body's own scroll state**, not the transcript's: `_e.value = Ee.scrollTop > 0` is assigned from the panel body's scroll handler, and its CSS drops `mask-image: linear-gradient(to bottom, transparent, black var(--menu-scroll-fade))` on `.dock-work-body` when it is set. The fork's `panel-goal` element was present all along — only the state class and the mask were missing. `DockWorkPanel.vue` now tracks its body's `scrollTop > 0` (re-read when the panel's kind changes, as upstream's `be()` does) and carries the same mask.

**Element rules can be scoped to a surface.** Three of the remaining signatures — `SPAN.hint`, `BUTTON.is-on.ui-switch`, `SPAN.ui-switch__thumb` — are shared by every settings row, so allowlisting them globally would have retired the element comparison of every toggle on every surface, including the surfaces this port has already brought to zero. `compare.mjs` element entries now accept an optional `scenes` list (globs over the finding's `combo::scene`), and those three are scoped to the Advanced walk state, where the missing element is upstream's unported `Auto-fold messages` row (its label and description are already allowlisted as the rejected turn-folding decision). The `Advanced` panel's other rows stay compared, and a row that later goes missing still lands as a `missing-text` blocker on its label — the element rule does not cover text.

**The Agent tab's thinking row is a Switch, not a value.** The last four element blockers, one per desktop combination, named `SPAN.mono.rvalue`: upstream prints "No default model" in the thinking-by-default row's value cell, while the fork renders a working switch bound to the daemon's thinking default (`ui-switch is-on`, `aria-checked="true"` in the captured state). Allowlisted as a recorded divergence, scoped the same way, for the same reason the signature is shared.

**The Chinese sidebar tooltip was never covered by the English rule.** The zh capture carries the expand-all tooltip's text as a hash id (`x-ad7828a7a0`) rather than the slug the English entry matches, so the entry written for the English capture did not suppress the Chinese one. Same measured cause — the fork's bubble is unmounted by the time the harness's scroll normalisation runs and upstream's survives it — recorded as a second entry.

**The gate is green.** `.tmp/wdfull25` (06:07–06:31, the same sixteen single-app jobs on the 06:06 bundle): blocker **0**, warnings 12 513, info 13 047, suppressed 570; `check-web-port-closeout.mjs --version 0.41 --run .tmp/wdfull25/final` exits 0 with 853 captured surfaces' page-source pairs verified, and all 116 requirement groups report `requirements-met`. The sixteen blockers of the previous run were the last of 636 → 163 → 120 → 106 → 16 → **0**.

**What the pre-fix measurement showed for the three scenes.** Rebuilt the fork bundle at HEAD without this pass's two source edits (`.tmp/prefix-web`) and ran the three area scenes against it with the upstream side captured beside them: every requirement of all three scenes is met on **both** apps there — 4 groups for the settings scene, 2 for the sidebar scene, 3 for the composer scene. So the scenes are invariants over the three areas rather than regression tests for this pass: what those three areas differed in was element and text markup, which is what the sixteen blockers above were.

**The scenes were then shown to fail on a bundle where the behaviour they name is absent**, which is what the "fails before the fix" check is for. One scratch build (`.tmp/broken-web`, served nowhere) with three one-line breaks — the dialog's overlay click no longer closes it, the sidebar's collapse-all control is inert, the composer's permission pill no longer opens its menu — and the same three scenes run against it. On the **fork** side they report exactly the requirements tied to the broken behaviour and nothing else: `composer-permission-menu` and `composer-permission-rows` (`.perm-dropdown` 0 rendered), `sidebar-groups-collapsed` (`.group-sessions` still 1), `settings-dialog-dismissed` (`.ui-dialog` still 1) — 4 blockers — while every other requirement in the same scenes stays met and the **upstream** side keeps all six groups. Source reverted, bundle discarded.

**One behaviour of this pass has no scene of its own.** The dock panel's `body-scrolled-up` mask is a scroll state of the panel's own body, and at a desktop viewport the Bash panel's body does not overflow, so a scene that scrolls it would have asserted nothing and failed on both apps (tried: `.dock-work-panel.body-scrolled-up` 0 rendered on both, so the scene was not kept). It stays covered by the walk's element comparison — the class is what the mobile goal state named before the fix — and by the CSS itself.
