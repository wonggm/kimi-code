...
- Subagent panel: 
  - [x] subagent status remained as running eventho task completed (stale, effectively), 
  - [x] clicking into subagent preview pane shows empty (the stale running ones only), they should stay and categorised as "completed", "running" (just like the TUI) subagent output as in "open Task", ui render ideally synchronised with main interface.
  - [x] sometimes clicking on subagent pane opens with content; sometimes opening empty
...
- [x] reloading webpage removes preview pane contents, 
- [] reloading webpages removes cache hit rate, and stales goal mode pill window's status (turns, tokens, time elapsed). status is stale and not live. tokens should show total of main agent + subagents(s)
...
- [] bug where steered message is missing when switching session: ctrl+s to send steer message --> switch session --> switch back --> steer message not seen
- [] changed files inconsistent, can be more feature rich (what's default upstream behaviour?)
### URGENT
- [x] dock pill above input box, weird underline at each pill
